...
- **Phase 11** — from mock user-panel exercises: **idea upvotes** (11.1, D73), **idea comments/discussion** (11.2, D74), **idea date + location** (11.3, D75), and **panel round 2** (11.4, D76) = comment push notifications + an "Upcoming" group-list section + picker "chosen at random from N" provenance. Deferred from round 2: RSVP/"I'm in" and a native mobile date-picker (needs a custom dev build). Weighted picker still declined.

Test suite green at last local run: typecheck (6) + lint (6) + format; unit (validation 80 / api-client 170 / core 21); **pgTAP 191**; **Playwright 59**; live probes run_picker 9/9, send-push 11/11, delete-account 10/10.

**⚠️ CI not running:** repo is private and GitHub Actions **included minutes are exhausted** — runs stopped being created ~2026-06-18. The new CI workflow can't be validated until minutes reset / spending limit raised / **repo made public** (recommended — unlimited Actions for public repos). All gate commands pass locally.

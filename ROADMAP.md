...
- [x] **OQ-4: Visual design direction.** Resolved → "Pop" direction: violet brand (c-purple) + pink accent (c-pink), Plus Jakarta Sans (web), three-circle "huddle" logo. Web tokens in `globals.css` `@theme`; mobile palette in `apps/mobile/src/lib/theme.ts`. Mobile keeps the native system font for now (parity-font deferred). Applied across both apps.
